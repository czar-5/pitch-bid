...
function TeamDetail() {
  ...
  const isTeamManager = !!membersQ.data?.some(
    (m) =>
      ...
      (m.user_id === user?.id ||
        (!!userEmail && m.profile?.email?.trim().toLowerCase() === userEmail)),
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border bg-card p-6 flex items-center gap-4">
        <div
          className="h-20 w-20 rounded-xl flex items-center justify-center font-bold text-2xl"
          style={{ background: team.primary_color ?? undefined }}
        >
          {team.logo_url ? (
            <img src={team.logo_url} alt="" className="h-full w-full rounded-xl object-cover" />
          ) : (
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Cannot be undone. The team will also be removed from any auctions it has been
                      added to.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
...
function AddMemberForm({
  ...
  const [role, setRole] = useState<"manager" | "co_manager">(
    allowManagerRole ? "manager" : "co_manager",
  );
  const add = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase
        .from("team_members")
        .insert({ team_id: teamId, user_id: profile.id, membership_role: role });
      if (error) throw error;
    },
    onSuccess: () => {
      toast.success(role === "manager" ? "Manager added" : "Co-manager added");
      setEmail("");
      onAdded();
    },
    onError: (e: Error) => toast.error(e.message),
