CREATE OR REPLACE FUNCTION public.get_next_player(_auction_id uuid)
 RETURNS TABLE(player_id uuid, name text, role text, photo text, batting_style text, bowling_style text, matches integer, runs integer, wickets integer, batting_avg numeric, batting_sr numeric)
 LANGUAGE sql
